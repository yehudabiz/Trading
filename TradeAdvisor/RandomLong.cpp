...
RandomLong::RandomLong (int symbol, int period, double pip, double sl, double tp, double friquency)
{
	m_max_size = 200;
	m_pip = pip;
	m_sl = (int)sl;
...
void RandomLong::AskAdvise(int& advise, double& stop_loss, double& take_profit)
{
	advise = 0;
	stop_loss = 0;
	take_profit = 0;
...
void RandomLong::UpdatePrice(Bar bar)
{
	bool is_new = (m_source_data.Update(bar)) == UPDATE_NEW;
}

void RandomLong::Deinit()
{
}
